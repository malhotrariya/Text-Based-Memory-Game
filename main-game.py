...
def generateBoard(words):
    ...
    cards = []
    for word in words:  # Create pairs of cards with words and a flag indicating if the card is flipped
        card1 = [word, False]
        card2 = [word, False]
        cards.append(card1)
        cards.append(card2)
    random.shuffle(cards)  # Shuffle the cards for randomness
    return cards  # Return the 2D list (board)

def displayBoard(board):
    # Function to display the current state of the memory game board
...
def getFlippedCards(board):
    # Function to get the indexes of flipped cards on the board
    result = []
    for index in range(len(board)):
...
def pickIndex(board):
    # Function to prompt the user to pick a card index
    while True:
